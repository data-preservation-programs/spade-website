...
const checkValue = field => {
  ...
  if (type === 'string') {
    if (fieldType === 'json') {
      try { JSON.parse(value) } catch (e) { state = 'error' }
    }
  }
  return { state, validation: state === 'error' ? 'malformed' : false }
}
...
const checkRequired = field => {
  ...
  const type = typeof value
  let state = 'not-started'
  if (type === 'string' || type === 'boolean' || type === 'string') {
...
export const useValidateField = field => {
  ...
  if (field.validate && field.type !== 'array') {
    ...
    if (check.state === 'not-started') { check = checkValue(field) }
